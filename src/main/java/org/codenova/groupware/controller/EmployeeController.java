...
public class EmployeeController {
    ...
    @PostMapping
    @Transactional
    public ResponseEntity<?> postEmployeeHandle(@ModelAttribute @Valid AddEmployee addEmployee,
                                                BindingResult result) {
        if(result.hasErrors()) {
            return ResponseEntity.status(400).build(); //bad request : 서버가 클라이언트 오류를 감지해 요청을 처리할 수 없는 코드
        }
        ...
        serialRepository.save(found); // 수정할때 다른 메서드가 존재하지 않고



        return ResponseEntity.status(201).body(employee); // created 요청이 성공적으로
    }
    ...
    public ResponseEntity<Employee> getEmployeeDetailHandle(@PathVariable String id) {
        ...
        if(employee.isEmpty()) {
            return ResponseEntity.status(404).body(null); // not found : 서버가 요청받을 리소스를 찾을 수 있다는 것을 의미
        }
